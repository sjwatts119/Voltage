@if($attachments->count() === 2)
    <div class="grid gap-4 grid-cols-1 md:grid-cols-2 mt-2 w-full lg:w-2/3 xl:w-1/2 2xl:w-1/3">
        @foreach($attachments as $attachment)
            <div class="relative">
                <x-thumbnail :attachment="$attachment" :currentIndex="$loop->index" classes="rounded-lg w-full h-[250px] object-cover cursor-pointer transition-transform duration-300 hover:scale-105" />
            </div>
        @endforeach
    </div>
        ...
        @foreach($attachments as $attachment)
            @if($loop->first)
                <div class="relative md:row-span-2 h-[250px] md:h-[266px]">
                    <x-thumbnail :attachment="$attachment" :currentIndex="$loop->index" classes="rounded-lg w-full h-full object-cover cursor-pointer transition-transform duration-300 hover:scale-105" />
                </div>
            @else
...
@elseif($attachments->count() > 3)
    ...
    <div class="grid gap-4 grid-cols-1 md:grid-cols-2 mt-2 w-full lg:w-2/3 xl:w-1/2 2xl:w-1/3">
        @foreach($attachments->take(3) as $attachment)
            <div class="relative">
                <x-thumbnail :attachment="$attachment" :currentIndex="$loop->index" classes="rounded-lg w-full h-[250px] md:h-[175px] object-cover cursor-pointer transition-transform duration-300 hover:scale-105" />
            </div>
        @endforeach
        <div class="relative">
            ...
            <x-thumbnail :attachment="$attachments[3]" :currentIndex="3" classes="rounded-lg w-full h-[250px] md:h-[175px] object-cover" />
        </div>
    </div>
@endif
