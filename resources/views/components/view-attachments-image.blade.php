@if($attachments->count() === 1)
    <div class="relative mt-2 inline-block">
        <div class="absolute inset-0 bg-gray-900/50 opacity-0 hover:opacity-100 transition-opacity duration-300 rounded-lg flex items-center justify-center w-full h-full">
            <a href="{{ asset('storage/attachments/' . $attachments[0]->attachment_path) }}" download>
        ...
        <img src="{{ asset('storage/attachments/' . $attachments[0]->attachment_path) }}" alt="An image sent by {{ $attachments[0]->message->user->name }}" class="rounded-lg max-w-full max-h-[250px] w-full h-auto object-contain" />
    </div>
@endif
