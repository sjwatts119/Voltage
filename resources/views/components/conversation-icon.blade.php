...
@if($currentConversation->is_group)
    ...
    <div class="flex items-center justify-center h-10 w-10 bg-pink-200 rounded-full antialiased">
    ...
    @foreach($currentConversation->users as $user)
        ...
        <div class="flex items-center justify-center h-10 w-10 rounded-full {{ $user->image ? '' : 'bg-purple-200' }}">
            @if($user->profile->profile_photo)
                <img src="{{ asset('storage/' . $user->profile->profile_photo) }}" alt="User Image" class="h-full w-full object-cover rounded-full">
            @else
                {{ $user->name[0] }}
            @endif
        </div>
