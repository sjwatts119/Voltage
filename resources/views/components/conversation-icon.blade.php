...
@if($currentConversation->is_group)
    ...
    <div class="flex items-center justify-center h-8 w-8 bg-pink-200 rounded-full antialiased">
...
@else
    ...
    @foreach($currentConversation->users as $user)
        ...
        <div class="flex items-center justify-center h-8 w-8 rounded-full {{ $user->image ? '' : 'bg-purple-200' }}">
            @if($user->image)
                <img src="{{ $user->image }}" alt="User Image" class="h-8 w-8 rounded-full">
            @else
                {{ $user->name[0] }}
            @endif
        </div>
